...
from student_management_app.models import Subjects, SessionYearModel, Students, Attendence, AttendenceReport, \
...
def staff_home(request):
    ...
    for student in students_attendance:
        attendance_present_count=AttendenceReport.objects.filter(status=True,student_id=student.id).count()
        attendance_absent_count=AttendenceReport.objects.filter(status=False,student_id=student.id).count()
...
def save_attendance_data(request):
    ...
    try:
        attendance=Attendence(subject_id=subject_model,attendence_date=attendence_date,session_year_id=session_model)
        attendance.save()

        for stud in json_sstudent:
             student=Students.objects.get(admin=stud['id'])
             attendence_report=AttendenceReport(student_id=student,attendence_id=attendance,status=stud['status'])
...
def save_updateattendance_data(request):
    ...
    try:
        for stud in json_sstudent:
             student=Students.objects.get(admin=stud['id'])
             attendance_report=AttendenceReport.objects.get(student_id=student,attendence_id=attendance)
